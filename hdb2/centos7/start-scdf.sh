...
start_scdf_admin () {
  ...
  if [ -z "`ps aux | grep spring-cloud-dataflow-server-local | grep -v grep`" ]; then
    kill -9 $(ps aux | grep spring-cloud-dataflow-server-local | grep -v grep | awk '{print $2}')
  fi
...
start_scdf_shell () {
  ...
  + jsonCar.frontLeftPsi + "","" + jsonCar.frontRightPsi + "","" +jsonCar.rearLeftPsi + "","" + jsonCar.rearRightPsi + "","" + jsonCar.make + "","" \
  + jsonCar.model + "","" + jsonCar.vin + "","" + jsonCar.year\nreturn csvString" | hdfs --hdfs.fs-uri=hdfs://centos7-namenode \
  --hdfs.directory=/demo --hdfs.file-name=demoData --hdfs.rollover=100000" --deploy' >> demo.cmd
