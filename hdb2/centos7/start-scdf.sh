...
start_scdf_shell () {
  ...
  echo -e 'stream create --name demo --definition "	http --server.port=9000 | scriptable-transform --script="import groovy.json.JsonSlurper\ndef jsonCar = new JsonSlurper().parseText( payload )\ndef csvString = jsonCar.amountOfFuel + "","" + jsonCar.tankCapacity + "","" + jsonCar.currentMileage + "","" + jsonCar.currentGear + "","" + jsonCar.currentRpm + "","" + jsonCar.eBrake + "","" + jsonCar.engTemp + "","" + jsonCar.insideTemp + "","" + jsonCar.outsideTemp + "","" + jsonCar.frontLeftPsi + "","" + jsonCar.frontRightPsi + "","" +jsonCar.rearLeftPsi + "","" + jsonCar.rearRightPsi + "","" + jsonCar.make + "","" + jsonCar.model + "","" + jsonCar.vin + "","" + jsonCar.year\nreturn csvString" --language=groovy | hdfs --hdfs.fs-uri=hdfs://centos7-namenode:8020 --hdfs.directory=/demo --hdfs.file-name=demoData --hdfs.rollover=100000" --deploy' >> demo.cmd  
